...
Template.allreports.onCreated(function () {
    ...
    templateObject.isBankingReport = new ReactiveVar(false);
...
Template.allreports.onRendered(() => {
    ...
    getVS1Data("ClockedHourReport").then(function (dataObject) {
        if (dataObject.length === 0) {
    ...
    getVS1Data("TFavReportBankingReport").then(function (dataObject) {
        if (dataObject.length > 0) {
            templateObject.isBankingReport.set(dataObject[0].data === 'true')
        }
    })
...
Template.allreports.events({
    ...
    'click .chkPLYearly': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('PLYearlyReport', 'true');
            templateObject.isPLYearly.set(true);
        } else {
            addVS1Data('PLYearlyReport', 'false');
    ...
    'click .chkClockedHourReport': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('ClockedHourReport', 'true');
            templateObject.isClockedHourReport.set(true);
        } else {
            addVS1Data('ClockedHourReport', 'false');
            templateObject.isClockedHourReport.set(false);
        }
    },
    ...
    'click .chkBinLocationsList': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('BinLocationsReport', 'true');
            templateObject.isBinLocations.set(true);
        } else {
            addVS1Data('BinLocationsReport', 'false');
    ...
    'click .chkTimeSheet': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('TimeSheetDetailsReport', 'true');
            templateObject.isTimeSheetDetails.set(true);
        } else {
            addVS1Data('TimeSheetDetailsReport', 'false');
    ...
    'click .chkPurchaseReport': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('PurchaseReport', 'true');
            templateObject.isPurchaseReport.set(true);
        } else {
            addVS1Data('PurchaseReport', 'false');
    ...
    'click .chkPurchaseSummaryReport': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('PurchaseSummaryReport', 'true');
            templateObject.isPurchaseSummaryReport.set(true);
        } else {
            addVS1Data('PurchaseSummaryReport', 'false');
    ...
    'click #formCheck-BankingReportFave': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('TFavReportBankingReport', 'true');
            templateObject.isBankingReport.set(true);
        } else {
            addVS1Data('TFavReportBankingReport', 'false');
            templateObject.isBankingReport.set(false);
        }
    },
    'click #formCheck-BankingReport': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('TFavReportBankingReport', 'true');
            templateObject.isBankingReport.set(true);
    ...
    // Manufacturing
    'click #formCheck-costReportFave': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
    ...
    'click .chkProductionWorksheetReport': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
    ...
    'click #formCheck-productionWorksheetReportFave': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
            addVS1Data('ProductionWorksheetReport', 'true');
            templateObject.isProductionWorkSheet.set(true);
    ...
    'click #formCheck-workOrderReportFave': function (event) {
        let templateObject = Template.instance();
        if ($(event.target).is(':checked')) {
...
Template.allreports.helpers({
    ...
    isFavorite: function () {
        ...
        if (isBalanceSheet ||
            ...
            Template.instance().isBankingReport.get() ||
