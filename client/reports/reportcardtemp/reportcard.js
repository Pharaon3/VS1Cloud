...
Template.reportcard.onCreated(function(){
    ...
    templateObject.filterData = new ReactiveVar();
...
Template.reportcard.onRendered(async function() {
  ...
  let tablename = templateObject.data.tablename
    ...
    templateObject.makeSubGroupData = function(data, updateCount){
    ...
    templateObject.makeNormalData = async function (data, updateCount = true) {
        let lowercasename = templateObject.data.lowercasename || templateObject.data.indexDBName.toLowerCase();
        if(data[lowercasename] && data[lowercasename].length > 0) {
            if(updateCount == true) {
                if(data.Params) {
            ...
            let tableData =  data[lowercasename];
            let calcFieldsIndex = [];
            for(let i = 0 ; i < tableData.length; i ++){
                tableData[i] = templateObject.data.datahandler(tableData[i]);
            }
            // let displaysettings = templateObject.reset_data.get();
            // displaysettings.forEach((item, index) => {
            //     if(item.calc == true) {
            //         calcFieldsIndex.push(index)
            //     }
            // });
            // tableData.forEach((item, index)=> {
            //     for(let p=0; p<calcFieldsIndex.length; p++) {
            //         item[calcFieldsIndex[p] - 1] = GlobalFunctions.showCurrency(item[calcFieldsIndex[p]-1])
            //     }
            // })
            templateObject.reportdata.set(tableData)
        } else {
            templateObject.reportdata.set('')
        }
    }
    templateObject.displayReportData = async function (data, updateCount = true) {
      if(templateObject.data.subGroup == false)
         templateObject.makeNormalData(data, updateCount);
      else
         templateObject.makeSubGroupData(data, updateCount);
      setTimeout(function () {
        $('#'+tablename).DataTable({
          //searching: false,
    ...
    // $(document).on('change', '#dateTo, #dateFrom', function () {
      let dateFrom = templateObject.data.dateFrom ? templateObject.data.dateFrom : GlobalFunctions.convertYearMonthDay($('#dateFrom').val());
      LoadingOverlay.show();
...
Template.reportcard.helpers({
  ...
  reportdata: ()=> {
    let templateObject = Template.instance();
    return templateObject.reportdata.get();
  },
...
Template.reportcard.events({
  ...
  'keyup #myInputSearch': function (event) {
    ...
    $('#' + tablename).DataTable().search(keyword).draw();
    templateObject.filterData.set(keyword.toLowerCase());
    if(event.keyCode == 13) {
      LoadingOverlay.show()
      $('#'+tablename).DataTable().destroy();
      $('#tablePrint').DataTable().destroy();
      setTimeout(function(){
        let dateFrom;
        if(templateObject.data.dateFrom)
            dateFrom = templateObject.data.dateFrom
        else
            dateFrom = GlobalFunctions.convertYearMonthDay($('#dateFrom').val());
...
Template.registerHelper('lookup', function (obj, key, val) {
    if(val != 'only')     return obj[key-1][val];
    else {
        return obj[key - 1];
    }
});
