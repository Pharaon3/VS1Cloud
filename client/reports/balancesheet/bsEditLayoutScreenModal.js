...
import layoutEditor from "../profitandloss/layoutEditor";
...
import './bsEditLayoutScreenModal.html';
...
Template.bseditlayoutscreen.onCreated(function () {
    const templateObject = Template.instance();
...
function getFlatData(data, flatData, level) {
    let sum = 0;
    for (let i = 0; i < data.length; i ++) {
        let item = data[i];
        let accountTree = ' '.repeat(level * 5) + item.name;
        let headerAccountTotal = 0;
        let subAccountTotal = 0;
        let id = flatData.length + 1;
        let accName = '';
        let accountNumber = ''
        let sortId = 0;
        let totalAssetLiability = 0;
        let totalCAssetLiability = 0;
        let typeId = 0;

        if (item.data) {
            accName = item.data['ACCNAME'];
            accountNumber = item.data['AccountNumber'];
            sortId = item.data['SortID'];
            typeId = item.data['TypeID'];
            totalAssetLiability = item.data['Total Asset & Liability'];
            totalCAssetLiability = item.data['Total Current Asset & Liability'];
            subAccountTotal = item.data['Sub Account Total'];
            headerAccountTotal = item.data['Header Account Total'];
        }
        sum += subAccountTotal;
        if (item.name.indexOf('Total') === 0) {
            headerAccountTotal = sum;
        }
        ...
        flatData.push({
            ...
            'Header Account Total': headerAccountTotal,
            'ID': id,
            'SortID': sortId,
            'Sub Account Total': subAccountTotal,
            'Total Asset & Liability': totalAssetLiability,
            'Total Current Asset & Liability': totalCAssetLiability,
            'TypeID': typeId
        });

        if (item.children && item.children.length > 0) {
            sum += getFlatData(item.children, flatData, level + 1);
        }
    }
    return sum;
}

Template.bseditlayoutscreen.onRendered(function () {
    const templateObject = Template.instance();
    ...
    templateObject.displayReportData = async function(data) {
        templateObject.reportData.set(data);
        if (Object.keys(data).length === 0) return;
        if (!data.balancesheetreport) return;

        let result = [];
        let parents = [];
        for (let i = 0; i < data.balancesheetreport.length; i ++) {
            let item = data.balancesheetreport[i];
            let name = item['Account Tree'];
            let level = (name.length - name.trim().length) / 5;
            let newItem = {
                name: name.trim(),
                id: name.trim(),
                level,
            ...
            if (level > 0) {
                parents[level-1].push(newItem);
            } else {
                if (!name.trim()) continue;
                result.push(newItem);
            }
            parents[level] = newItem.children;
        }
    ...
    templateObject.setDateAs = (dateFrom = null) => {
        ...
        if(templateObject.data.dateFrom)
    ...
    templateObject.getReportData(
        initialReportLoad, 0,
        templateObject.data.dateFrom ? templateObject.data.dateFrom : GlobalFunctions.convertYearMonthDay($('#dateFrom').val()),
...
Template.bseditlayoutscreen.events({
    ...
    "click .saveProfitLossLayouts": function(event) {
        let pnlLayoutTree = $('#pnlLayoutTree');
        let data = pnlLayoutTree.tree("toJson");
        data = JSON.parse(data);
        let flatData = [];
        getFlatData(data, flatData, 0);
        let indexdbname = Template.instance().data.indexDBName;
        let reportData = Template.instance().reportData.get();
        reportData.balancesheetreport = flatData;
        addVS1Data(indexdbname, JSON.stringify(reportData));
    },
...
Template.bseditlayoutscreen.helpers({
    companyname: () => {
