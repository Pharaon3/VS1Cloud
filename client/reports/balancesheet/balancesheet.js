    ...
    templateObject.getReportDataRecord = function(data) {
        ...
        if(data!='') {
            ...
            if(!dataList[1] && !dataList[2]) {
                ...
                if (data["Total Current Asset & Liability"]) {
                    tmp = data["Total Current Asset & Liability"];
                    dataList[2] = (tmp >= 0) ? GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"table-cells text-bold") : GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"text-danger text-bold");
                }
                else if(data["Total Asset & Liability"]){
                    tmp = data["Total Asset & Liability"];
                    dataList[2] = (tmp >= 0) ? GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"table-cells text-bold") : GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"text-danger text-bold");
                }
            }
            else if(dataList[2]){
                ...
                dataList[2] = (tmp >= 0) ? GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"text-primary text-bold") : GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"text-danger text-bold");
            }
            else if(dataList[1]){
                tmp = dataList[1];
                dataList[0] = GlobalFunctions.generateSpan(dataList[0],"text-primary");
                dataList[1] = (tmp >= 0) ? GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"text-primary") : GlobalFunctions.generateSpan(GlobalFunctions.showCurrency(tmp),"text-danger", );
            }
            for(let i = 0; i < 3; i ++)
                dataList[i].indent += data["TypeID"];
        }else {
            dataList = [
                "", "", ""
            ]
...
Template.balancesheetreport.events({
    "click td": function (event) {
        let transactiontype = $(event.target).closest("td").attr("data-column");
        let dateFrom = '01/01/1899';
        let dateTo = $("#dateTo").val();
        if (transactiontype)
            window.open(`/balancetransactionlist?accountName=${transactiontype}&toDate=${dateTo}&fromDate=${dateFrom}&isTabItem=false`, "_self")
        else
            swal({
                title: 'Information',
...
Template.balancesheetreport.helpers({
    ...
    searchFunction: function () {
        return reportService.getBalanceSheetReportByWord;
    },
});
