...
Template.executivesummaryreport.onRendered(() => {
  ...
  templateObject.setFieldVariance = function (fieldVal1, fieldVal2, fieldSelector) {
    ...
    templateObject.getDashboardExecutiveData = async function (dateAsOf, dateChanged) {
        templateObject.setDateAs( dateAsOf );
        getVS1Data('TDashboardExecData1').then(function (dataObject) {
            if (dataObject.length == 0) {
              LoadingOverlay.show();

              reportService.getCardDataReport(dateAsOf).then(async function (data) {
                    await addVS1Data('TDashboardExecData1', JSON.stringify(data));
                    templateObject.displayExecutiveData(data);
                    LoadingOverlay.hide();
                }).catch(function (err) {
  ...
  templateObject.displayExecutiveData = async (data) => {
