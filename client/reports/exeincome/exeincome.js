...
Template.exeincomereport.onCreated(function () {
  ...
  let reset_data = [
    ...
    { index: 1, label: "Account Name", class: "colAccountName", active: true, display: true, width: "500", calc: false },
    { index: 2, label: "Account No", class: "colAccountNo", active: true, display: true, width: "500", calc: false },
  ];
  templateObject.displaysettings.set(reset_data);
  templateObject.getReportDataRecord = function (data) {
    var dataList = [];
    if (data != "") {
      dataList = [data.ID || "", data.ACCNAME || "", data.AccountNumber || ""];
    } else {
      dataList = ["", "", ""];
    }
    return dataList;
...
Template.exeincomereport.onRendered(() => {
  ...
  //   let strDate = "";
  //   if (pDate.length == 10) strDate = pDate;
  //   else strDate = pDate.substring(1, 11);
  //   arrDate = strDate.split("-");
  //   templateObject.setMonthsOnHeader(arrDate[0], eval(arrDate[1]) - 1, arrDate[2]);
  //   templateObject.getIncomeReports(GlobalFunctions.convertYearMonthDay($("#dateFrom").val()), false);
...
Template.exeincomereport.helpers({
  ...
  searchFunction: function () {
    return reportService.getIncomeReportByKeyword;
  },
  //   titleMonth1: () => {
...
Template.exeincomereport.events({
  "click #tblIncomeExecutive tbody tr": function (event) {
  },
  ...
  //     $(event.currentTarget).attr("checked", $(event.currentTarget).prop("checked"));
  //   },
