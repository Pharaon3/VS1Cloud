...
Template.appointmentlist.onRendered(async function () {
    ...
    templateObject.getAllAppointmentListData = async function () {
        ...
        getVS1Data("TAppointmentList").then(async function (dataObject) {
            if (dataObject.length == 0) {
                sideBarService
                    .getTAppointmentListData(prevMonth11Date, toDate, true, initialReportLoad, 0)
                    .then(function (data) {
                                                    ...
                                                    if (smsSettings.twilioAccountId !== "" && smsSettings.twilioAccountToken !== "" && smsSettings.twilioTelephoneNumber !== "") {
                                                        const sentSMSs =
                                                            recentSMSLogs.sms_messages.filter(
                                                                (message) => message.from === "+" + smsSettings.twilioTelephoneNumber.replace("+", "") && message.to === "+" + data.tappointmentlist[i].Mobile.replace("+", "")
                                                            ) || "";
                                                        const receiveSMSs =
                                                            recentSMSLogs.sms_messages.filter(
                                                                (message) => message.to === "+" + smsSettings.twilioTelephoneNumber.replace("+", "") && message.from === "+" + data.tappointmentlist[i].Mobile.replace("+", "")
                                                            ) || "";
...
Template.appointmentlist.events({
                    ...
                    for (let i = 0; i < data.tappointmentlist.length; i++) {
                        ...
                        if (accessLevel) {
                            if (data.tappointmentlist[i].CUSTFLD13 === "Yes" && data.tappointmentlist[i].CUSTFLD11 === "" && data.tappointmentlist[i].Active == true) {
                                ...
                                if (smsSettings.twilioAccountId !== "" && smsSettings.twilioAccountToken !== "" && smsSettings.twilioTelephoneNumber !== "") {
                                    const sentSMSs =
                                        recentSMSLogs.sms_messages.filter((message) => message.from === "+" + smsSettings.twilioTelephoneNumber.replace("+", "") && message.to === "+" + data.tappointmentlist[i].Mobile.replace("+", "")) ||
                                        "";
                                    const receiveSMSs =
                                        recentSMSLogs.sms_messages.filter((message) => message.to === "+" + smsSettings.twilioTelephoneNumber.replace("+", "") && message.from === "+" + data.tappointmentlist[i].Mobile.replace("+", "")) ||
                                        "";
                                    let currentSentSMSDate = null;
                                    let nextSentSMSDate = null;
                                    if (sentSMSs.length > 0) {
    ...
    "click .chkAppointment": async function () {
        ...
        $(".chkAppointment:checked").each(async function () {
            var chkIdLine = $(this).closest("tr").attr("id");
            if (selectAppointment.length > 0) {
                let checkAppointStartEnd = selectAppointment.filter((item) => parseInt(item.id) == parseInt(chkIdLine));
