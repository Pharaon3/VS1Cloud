    ...
    getVS1Data('TInvoiceEx').then(function(dataObject) {
      if(dataObject.length > 0) {
        ...
        for(let i = 0; i< useData.length; i++) {
          if(useData[i].fields.ID == currentID) {
            added = true;
            $('#edtSaleCustField1').val(useData[i].fields.SaleCustField1);
            $('#edtSaleCustField2').val(useData[i].fields.SaleCustField2);
            $('#edtSaleCustField3').val(useData[i].fields.SaleCustField3);  
          }
        }
        if(added = false) {
