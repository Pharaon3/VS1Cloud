...
Template.closingdates.onRendered(() => {
  ...
  /**
   * Preload data from IndexDB / Damien
   */
  templateObject.preloadData = function() {
    getVS1Data('VS1_ClosingDates').then(function (dataObject) {
      if (dataObject.length == 0) {
        sideBarService.getVS1ClosingDates().then(function (data) {
          ...
          loadClosingDates(data);
        }).catch(function (err) {

        });
      } else {
        loadClosingDates(dataObject);
      }
    }).catch(function() {
      sideBarService.getVS1ClosingDates().then(function (data) {
        addVS1Data('VS1_ClosingDates', JSON.stringify(data));
  ...
  templateObject.preloadData();

  function loadClosingDates(data) {
    if(data) {
      data = JSON.parse(data[0].data);

      if(data && data.ProcessLog && data.ProcessLog.ClosingDates) {
        data = data.ProcessLog.ClosingDates;
      }
