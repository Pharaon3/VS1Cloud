...
Template.supplierpaymentcard.onCreated(() => {
  ...
  templateObject.isForeignEnabled.set(isFxCurrencyLicence);
});
...
Template.supplierpaymentcard.onRendered(() => {
 ...
 $("#edtSupplierName").attr("readonly", true);
  ...
  $(document).on("click", "#tblSupplierlist tbody tr", function (e) {
        ...
        $('#edtSupplierName').val(tableCustomer.find(".colCompany").text());
        $('#supplierListModal').modal('toggle');

        $('#edtSupplierEmail').val(tableCustomer.find(".colEmail").text());
        $('#edtSupplierEmail').attr('customerid', tableCustomer.find(".colID").text());
  ...
  templateObject.fetchSupplierPayment = async function () {
      ...
      $('.appliedAmount').text($('#edtApplied').val());
  ...
  "click .printConfirm": async function (event) {
    ...
    'click #printModal .btn-check-template': async function (event) {
        ...
        setTimeout(async function(){
            ...
            let result = await exportSalesToPdf(template,template_number);
            if(result == true)
            {
...
export function onExchangeRateChange(e = null) {
 const templateObject = Template.instance();
  ...
  const foreignValue = parseFloat($('.edtForeignAmount').val().replace(/[^0-9.-]+/g, "")) ;
  ...
  $('#edtForeignAmount').val(currency + foreignValue);
...
export function calculateVariation(foreignChanged = 0 /* 0 : foreign Changed */) {

    const _foreignAmount = $('#edtForeignAmount').val();
    const _paymentAmount = $('.edtPaymentAmount').val();
    const _applied = $('#edtApplied').val();

    const currency = getCurrentCurrencySymbol();
    let foreignAmount = isNaN(_foreignAmount) ? utilityService.removeCurrency(_foreignAmount) : _foreignAmount;
    let paymentAmount = isNaN(_paymentAmount) ? utilityService.removeCurrency(_paymentAmount) : _paymentAmount;
    const applied  = isNaN(_applied) ? utilityService.removeCurrency(_applied) : _applied;
    const isNegative = utilityService.isNegative(foreignAmount);

    let cal = 0.00;
    const rate = parseFloat($("#exchange_rate").val());
    if(!foreignChanged) {
        cal = (foreignAmount / rate).toFixed(2);
        $('.edtPaymentAmount').val(utilityService.modifynegativeCurrencyFormat(cal));
        paymentAmount = cal;
    }
    else {
        cal = (paymentAmount * rate).toFixed(2);
        $('#edtForeignAmount').val(currency + cal);
        foreignAmount = cal;
    }

    cal = (paymentAmount - applied).toFixed(2);
    //const calculatedVariation = isNegative ? `-${currency}${cal}` : `${currency}${cal}`;
    const calculatedVariation = isNegative ? `-${cal}` : cal;

    $('#edtVariation').val(utilityService.modifynegativeCurrencyFormat(calculatedVariation));
