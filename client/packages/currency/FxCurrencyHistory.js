...
Template.FxCurrencyHistory.onCreated(function () {
  ...
  templateInstance.getDataTableList = function(data) {
    var dataList = [
      ...
      // ),
      //moment(data.MsTimeStamp).format("DD/MM/YYYY") || "",
      '<span style="display:none;">' + (data.MsTimeStamp != '' ? moment(data.MsTimeStamp).format("YYYY/MM/DD") : data.MsTimeStamp).toString() + '</span>' +
      (data.MsTimeStamp != '' ? moment(data.MsTimeStamp).format("DD/MM/YYYY") : data.MsTimeStamp).toString(),
      data.Code || "-",
      data.BuyRate || "-",
      data.SellRate || "-",
    ];
...
Template.FxCurrencyHistory.events({
  ...
  "click .btnRefresh": function () {
    ...
    //Meteor._reload.reload();
    $(".fullScreenSpin").css("display", "inline-block");
    taxRateService.getCurrencyHistory(initialBaseDataLoad, 0, false).then(function(dataReload) {
      addVS1Data('TCurrencyRateHistory', JSON.stringify(dataReload)).then(function(datareturn) {
        taxRateService.getCurrencyHistory(initialBaseDataLoad, 0, false).then(async function(dataCurrencyList) {
          await addVS1Data('TCurrencyRateHistory', JSON.stringify(dataCurrencyList)).then(function(datareturn) {
