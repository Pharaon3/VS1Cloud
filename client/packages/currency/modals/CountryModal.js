...
Template.CountryModal.onCreated(function () {
  ...
  let headerStructure = [
    ...
    { index: 1, label: 'Country', class: 'colCountry', active: true, display: true, width: "1350" },
...
Template.CountryModal.events({
  ...
  "click #tblCountryPopList tbody tr": (e) => {
    ...
    const countryName = $(e.currentTarget).find("td.colCountry").text();
  ...
  'click .btnAddNewCountry': function (){
    $('div#newCountryModal').modal('show');
},
});
...
Template.CountryModal.helpers({
  ...
  apiFunction:function() {
    let sideBarService = new SideBarService();
    return sideBarService.getCountries;
  },
  searchAPI: function() {
    return sideBarService.getOneCountryByName;
  },
  service: ()=>{
    let sideBarService = new SideBarService();
