...
import '../quote/frm_quote_temp.html'
...
function generateHtmlMailBody(erpInvoiceId, stringQuery) {
...
Template.new_quote_temp.onCreated(() => {
  ...
  //   let checkEmailData = $('#edtCustomerEmail').val();
  //   let mailSubject = 'Quote ' + erpInvoiceId + ' from ' + mailFromName + ' for ' + customerEmailName;
  //   const htmlmailBody = generateHtmlMailBody(erpInvoiceId, stringQuery);
...
Template.new_quote_temp.onRendered(() => {
  ...
  //   }
  // }
  templateObject.setOneQuoteData = function (data) {
    let customerData = templateObject.clientrecords.get();
    let cust_result = customerData.filter(cust_data => {
      return cust_data.customername == data.fields.CustomerName
    });
...
Template.new_quote_temp.helpers({
  oneExAPIName: function () {
    let salesService = new SalesBoardService();
    return salesService.getOneQuotedataEx;
  },
  ...
  setTransData: () => {
    ...
    return function (data) {
      let dataReturn = templateObject.setOneQuoteData(data)
      return dataReturn;
    }
  },
      ...
      if (id_available == true) {
        ...
        if (customername.val() === '') {
          ...
          salesService.saveQuoteEx(objDetails).then(function (objDetails) {
            ...
            //   var htmlmailBody = generateHtmlMailBody(erpInvoiceId, stringQuery);
