...
import '../credit/frmcredit_card_temp.html';
...
Template.creditcard_temp.onCreated(() => {
...
Template.creditcard_temp.onRendered(() => {
  ...
  //   $(".transheader > #sltShipVia_fromtransactionheader").editableSelect();
...
Template.creditcard_temp.onRendered(function () {
  ...
  templateObject.setOneCreditData = (data) => {
    $(".fullScreenSpin").css("display", "none");
    let lineItems = [];
    let lineItemObj = {};
    let lineItemsTable = [];
    ...
    $(".transheader > #sltDept_fromtransactionheader").val(getDepartmentVal);
    ...
    return {record: creditrecord, attachmentCount: templateObject.attachmentCount.get(), uploadedFiles: templateObject.uploadedFiles.get(), selectedCurrency: creditrecord.currency}

  }

  

  templateObject.saveCredit = (data) => {
    ...
    setTimeout(function () {
      ...
      if (suppliername.val() === "") {
        ...
        let currentCredittemp = templateObject.temporaryfiles.get();
        let newCredittemp = [...currentCredittemp, objDetails];
        templateObject.temporaryfiles.set(newCredittemp);

        addVS1Data('TCreditTemp', JSON.stringify({tcredittemp: newCredittemp})).then(function () {
            ...
            addAttachment("Credit", "Supplier", objDetails.fields.ID || '', htmlmailBody, 'creditlist', 21,  'html-2-pdfwrapper', '', true)
  ...
  templateObject.updateCreditTemp = async function(objDetails) {
    return new Promise( (resolve, reject) => {
      ...
      templateObject.temporaryfiles.set(newTemp);
       addVS1Data('TCreditTemp', JSON.stringify({tcredittemp:newTemp})).then(function(){resolve()})
  })
}
});

Template.creditcard_temp.helpers({
  oneExAPIName: function () {
    let purchaseBoardService = new PurchaseBoardService();
    return purchaseBoardService.getOneCreditData;
  },
  ...
  setTransData: () => {
    ...
    return function (data) {
      let dataReturn = templateObject.setOneCreditData(data)
      return dataReturn;
    }
  },
  ...
  updateTransactionTemp:  function() {
      ...
      return async function(data) {
        await templateObject.updateCreditTemp(data)
      }
  },
});

Template.creditcard_temp.events({
