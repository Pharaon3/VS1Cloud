...
Template.receiptsoverview.onCreated(function () {
    ...
    templateObject.getDataTableList = function(data) {
        var dataList = [
            ...
            '<span style="display:none;">'+(data.DateTime !=''? moment(data.DateTime).format("YYYY/MM/DD"): data.DateTime)+'</span>'+(data.DateTime !=''? moment(data.DateTime).format("DD/MM/YYYY"): data.DateTime),
            // data.EmployeeName || '',
            utilityService.modifynegativeCurrencyFormat( data.TotalAmountInc ) || 0.0,
            data.RequestToEmployeeName || '',
            data.TripGroup || '',
            data.Description || '',
            // data.fields.Lines[0].fields.Reimbursement || '',
            data.RequestToEmployeeID || '',
            data.RefNo || '',
            data.EmployeeID || '',
            data.EmployeeName || '',
            // data.fields.Lines[0].fields.TaxCode || '',
            utilityService.modifynegativeCurrencyFormat( data.TotalTax ) || 0.0,
...
Template.receiptsoverview.onRendered(function () {
    ...
    function setCurrencySelect(e) {
        ...
        if (e.pageX > offset.left + $each.width() - 8) { // X button 16px wide?
            ...
            setTimeout(() => {
                $('#tblCurrencyPopList_filter .form-control-sm').get(0).focus()
              }, 500)
        } else {
...
*/
    ...
    templateObject.openReceiptClaimModal = function (selectedId) {
        let selectedClaim;
        accountService.getExpanseClaimListById(selectedId).then(function(data){
            selectedClaim = data.texpenseclaimlist[0];
            ...
            $('#viewReceiptModal .merchants').val(selectedClaim.RequestToEmployeeName);
            $('#viewReceiptModal .merchants').attr('data-id', selectedClaim.RequestToEmployeeID);
            $('#viewReceiptModal .chart-accounts').val(selectedClaim.Category);
            ...
            $('#viewReceiptModal .currencies').val(selectedClaim.ForeignExchangeCode);
            $('#viewReceiptModal .edtTotal').val(utilityService.modifynegativeCurrencyFormat( selectedClaim.TotalAmountInc ));
            setTimeout(() => {
                $('#viewReceiptModal .dtReceiptDate').val(selectedClaim.DateTime !=''? moment(selectedClaim.DateTime).format("DD/MM/YYYY"): selectedClaim.DateTime);
            }, 200);
...
Template.receiptsoverview.events({
    ...
    'click #tblReceiptList tbody tr td:not(:nth-child(2))': function (event) {
        let template = Template.instance();
        const selectedId = $(event.target).closest('tr').attr('id');
        var checkDeleted = $(event.target).closest('tr').find('.colStatus').text() || '';
        if(checkDeleted == "In-Active")
            swal('You Cannot View This Transaction', 'Because Its Status Is In-Active', 'info');
        else
    ...
    'click .btnRefresh': function () {
        ...
        accountService.getExpanseClaimList(prevMonth11Date, toDate, true, initialDataLoad, 0, false).then(function (expenseData) {
            addVS1Data('TExpenseClaimList', JSON.stringify(expenseData)).then(function (datareturn) {
                sideBarService.getAllSuppliersDataVS1(initialBaseDataLoad, 0).then(function (dataReload) {
...
Template.receiptsoverview.helpers({
    ...
    searchAPI: function() {
        let accountService = new AccountService();
        return accountService.getExpanseClaimListByEmployeeName;
    },
