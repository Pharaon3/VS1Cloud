...
let deliverychart = {
  fields: {
    Active: true,
    ChartGroup: "Delivery",
    ChartName: "Delivery Overview",
    ID: 1011,
    _chartSlug: "delivery_overview",
  },
};
...
export class ChartService extends BaseService {
  async checkChartToDisplay() {
    ...
    //   if (dashboardpreferences.tvs1dashboardpreferences?.length) {
    //     dashboardpreferences.tvs1dashboardpreferences.forEach((chart) => {
    ...
    if (chartList.length > 0) {      
      ...
      setTimeout(() => {
        chartList.forEach((chart) => {
          ...
          // $(`[key='${chart.fields.ChartName}']`).addClass("chart-visibility");
          if ($(`[key='${chart.fields.ChartName}']`).length === 0 || chart.fields.ChartGroup != _chartGroup) return
          // $(`[key='${chart.fields.ChartName}']`).attr("pref-id", chart.fields.ID);
          
          $(`[key='${chart.fields.ChartName}']`).attr(
            "chart-id",
            chart.fields.ID
          );
          
          // Default charts
          let defaultClass = $(`[key='${chart.fields.ChartName}']`).attr(
            "data-default-class"
          );
          let defaultPosition = $(`[key='${chart.fields.ChartName}']`).attr(
          ...
          $(`[key='${chart.fields.ChartName}']`).addClass(defaultClass);
          $(`[key='${chart.fields.ChartName}']`).attr(
            "position",
            storeObj ? storeObj.position : defaultPosition
          );
          $(`[key='${chart.fields.ChartName}']`).attr("width", "100%");
          $(`[key='${chart.fields.ChartName}']`).css(
            "height",
            storeObj && storeObj.height && storeObj.height != 0
              ? storeObj.height + "px"
              : "auto"
          );
          $(`[key='${chart.fields.ChartName}'] .ui-resizable`).css(
            "width",
          ...
          if (
            ...
            $(`[key='${chart.fields.ChartName}'] .chartShowOption`).prop(
              "checked",
              true
            );
            $(`[key='${chart.fields.ChartName}'] .chart-img`).addClass("d-none");
            $(`[key='${chart.fields.ChartName}'] .chart-canvas`).removeClass("d-none");
            $(`[key='${chart.fields.ChartName}']`).removeClass("hideelement");
            ...
            // ) {
            //   $(`[key='${chart.fields.ChartName}']`).addClass("hideelement");
            // }
          } else {
            $(`[key='${chart.fields.ChartName}'] .chartShowOption`).prop(
              "checked",
            ...
            $(`[key='${chart.fields.ChartName}']`).addClass("hideelement");
            $(`[key='${chart.fields.ChartName}'] .chart-canvas`).addClass("d-none");
            $(`[key='${chart.fields.ChartName}'] .chart-img`).removeClass("d-none");
          }
          // $(`[key='${chart.fields.ChartName}']`).attr(
          //   "pref-id",
          //   chart.fields.ID
        ...
        const chartEvent = new Event("chartDisplayEvent");
