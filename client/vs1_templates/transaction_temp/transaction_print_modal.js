...
Template.transaction_print_modal.events({
  ...
  "click #printModal #previewTemplate": function (event) {
    ...
    $('#closeTemplatePreview').data('template', '');
  ...
  "click #print_template_modal #closeTemplatePreview": function(event) {
    let current_template = $('#closeTemplatePreview').data('template');
    let defaultTemplates = $('.chooseTemplateModal .chkGlobalSettings:checked');
    let checkedTemplates = $("#printModal.show .chooseTemplateBtn:checked");
    let nextTemplate = '';
    if (checkedTemplates) {
      for (let i = 0; i < checkedTemplates.length; i++) {
        if ($(checkedTemplates[i]).data('id') == current_template) {
          if (i < checkedTemplates.length - 1) {
            nextTemplate = $(checkedTemplates[i+1]).data('id');
          }
        }
      }
    }
    ...
    if (localStorage.getItem("confirmPrintStatus") == 'preview') {
      localStorage.setItem("confirmPrintStatus", "print");
