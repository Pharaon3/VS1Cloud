...
Template.customerselector.onRendered(function() {
    ...
    templateObject.setCustomerByID = function(data) {
        ...
        let termsfieldname = '#sltTerms';
        if(templateObject.data.custid) {
          termsfieldname = termsfieldname + '_'+ templateObject.data.custid
        }
        $(termsfieldname).val(
    ...
    function setSupplierByID(data){
        ...
        $(termsfieldname).val(data.fields.TermsName || purchaseDefaultTerms);
    ...
    $(document).on('click', '#tblCustomerlist tbody tr', function(event) {
        ...
        $(termsfieldname).trigger('change');
        let termsField = $(termsfieldname);
        if(termsField && termsField.length > 0) {
            $(termsfieldname).val($(event.target).closest('tr').find('.'+ colTerms).text())
        }
        ...
        let accountNo = $(event.target).closest('tr').find('.colAccountNo').text();
        ...
        $("#edtCustomerAccountNo").val(accountNo);
