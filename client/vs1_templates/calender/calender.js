...
async function sendAppointmentEmail() {
    ...
    if (customerEmailCheck == true) {
        let customerDataName = $("#customer_appointment_calendar").val();
        let customerEmail = "";
...
Template.calender.onRendered(function() {
        ...
        refresh: {
            ...
            click: async function() {
                try{
                    ...
                    let updatedAppointmentList = await sideBarService.getAllAppointmentList(initialDataLoad, 0);
                    await addVS1Data("TAppointment", JSON.stringify(updatedAppointmentList));
                    let updatedTimeSheetList = await sideBarService.getAllTimeSheetList()
                    await addVS1Data("TTimeSheet",JSON.stringify(updatedTimeSheetList));
                    refreshPage();                                       
                }catch(err){
                    refreshPage(); 
                }
    ...
    templateObject.updateEvents = async (updatedEvent,isLeave) => {
    ...
    function renderEventClick(info){
        ...
        if (result.length > 0) {
            ...
            document.getElementById("customer_appointment_calendar").value = result[0].accountname;
    ...
    function renderCalendarDropEvent(event){
        ...
        if (FlowRouter.current().queryParams.leadid) {
            ...
            if (!leaveFlag) {
                ...
                $('#customer_appointment_calendar').click();
            }
        }
    }
    ...
    function renderEventDidMount(info){
        ...
        if(pattern.test(info.event._def.publicId)){
            ...
            if (parentDiv && parentDiv.nextElementSibling && parentDiv.nextElementSibling.tagName === 'DIV') {
    ...
    templateObject.renderNormalCalendar = function(slotMin, slotMax, hideDays) {
        ...
        setTimeout(() => {
            ...
            if (child1 != null) {
                ...
                $("#calendar-appointment .fc-toolbar-title").css("min-width", 275).css("text-align", "center");
    ...
    templateObject.getActualHours = function(){
        ...
        if (date2 != "" && endTime > startTime) {
            let hours = parseFloat(templateObject.diff_hours(endTime, startTime)).toFixed(2) || 1;
            return hours;
        } else {}
    }

    templateObject.getBookingHours = function(){
        let date1 = document.getElementById("dtSODate").value;
        let date2 = document.getElementById("dtSODate2").value;
        date1 = templateObject.dateFormat(date1);
        date2 = templateObject.dateFormat(date2);
    ...
    templateObject.sendStartAppointmentSMS = async function(){
        ...
        const customerPhone = $("#mobile").val();
            ...
            if ((smsCustomer || smsUser) && customerPhone != "0" && smsSettings.twilioAccountId) {
                ...
                if (!sendSMSRes.success) {
                    ...
                    $("#btnCloseStartAppointmentModal").click();
    ...
    templateObject.sendStopAppointmentSMS = async function(){
        ...
        if ((smsCustomer || smsUser) && customerPhone != "0" && smsSettings.twilioAccountId) {
            ...
            if (!sendSMSRes.success) {
                ...
                $('#btnCloseStopAppointmentModal').click();
                $("#btnAppointmentSubmit").click();
                $("#event-modal").modal("hide");
            }
        } else {
    ...
    //             //$('#customer_appointment_calendar').editableSelect('add', clientList[i].customername);
    //         }
    //     });
    // }
    ...
    $(document).ready(function() {
        $("#customer_appointment_calendar").editableSelect();
        $("#product-list").editableSelect();
        $("#product-list-1").editableSelect();
/*
        $("#customer_appointment_calendar").editableSelect().on("click.editable-select", function(e, li) {
			var $earch = $(this);
			...
			// });
			//
			// document.getElementById("customer_appointment_calendar").value = newJob[0].customername || '';
...
*/
    ...
    //     const productDataName = event.target.value || "";
    //     //var productDataID = el.context.value || "";
    //     // if(el){
    //     //   var productCostData = el.context.id || 0;
    //     //   $('#edtProductCost').val(productCostData);
    ...
    $(document).on("click", "#tblEmployeelist_appointments tbody tr", function(e) {
    ...
    $(document).on("click", ".appointmentCustomer #tblCustomerlist tbody tr", function(e) {
        ...
        document.getElementById("customer_appointment_calendar").value = $(this).find(".colCompany").text();
...
Template.calender.events({
    ...
    'click .calendar-appointment .days li': function(event) {
            ...
            if ($("#tActualStartTime").val() != "" && result[0].isPaused == "Paused" ) {
                ...
                appointmentService.saveTimeLog(objectData).then(function(data) {
                        ...
                        if (toUpdateID != "") {
                            ...
                            if (result[0].timelog != "") {
                                appointmentService.saveTimeLog(objectData).then(function(data) {
                                    appointmentService.saveAppointment(objectData1).then(async function(data1) {
                                        let index = appointmentData.map(function(e) {
                                                return e.id;
                                            })
                                        ...
                                        $(".fullScreenSpin").css("display", "none");
                                        $('.fc-refresh-button').addClass('btnRefreshAlert');
                                        templateObject.sendStartAppointmentSMS();
                                    }).catch(function(err) {
                                        swal({
                                            title: "Oops...",
                                            text: err,
            ...
            if (smsCustomer || smsUser) {
                swal({
                    ...
                    if (result.value) {
                        $("#chkSMSCustomer").prop("checked", false);
                        $("#chkSMSUser").prop("checked", false);
                        $("#btnEndActualTime").click();
                    }
                });
            } else $("#btnEndActualTime").click();
        } else {
            const templateObject = Template.instance();
            const smsSettings = templateObject.defaultSMSSettings.get();
            if (smsCustomer || smsUser) {
        ...
        if (paused == "Paused") {
            ...
            if (document.getElementById("tActualStartTime").value == "") {} else {
                ...
                swal({
                    ...
                    if (result.value) {
                        let hours = parseFloat(templateObject.getActualHours());
                        document.getElementById('txtActualHoursSpent').value = templateObject.timeFormat(hours) || "";
                        templateObject.sendStopAppointmentSMS();
                    } else if (result.dismiss === 'cancel') {
                        document.getElementById('tActualEndTime').value = "";
                        document.getElementById('txtActualHoursSpent').value = "0";
                    } else {
    ...
    'change #startTime': function() {
        const templateObject = Template.instance();
        templateObject.getBookingHours();
    },
    ...
    'change #tActualStartTime': function() {
        const templateObject = Template.instance();
        let hours = templateObject.getActualHours();
        document.getElementById('txtActualHoursSpent').value = templateObject.timeFormat(hours) || "";
    },
    ...
    'click #btnCopyAppointment': function(event) {
        ...
        if (updateID != 0) {
            let aStartDate = "";
            let aEndDate = "";
            let clientname = $('#customer_appointment_calendar').val() || "";
    ...
    "submit form#frmAppointment": async function(event) {
        ...
        if (createAppointment == false) {
                        ...
                        appointmentService.saveTimeLog(obj).then(function(data1) {
                            if (obj.fields.Description == "Job Completed") {
                                ...
                                if (Object.keys(updateData).length > 0) {
                                    appointmentService.saveTimeLog(updateData).then(function(result) {
                                        let data = {
                                            ...
                                            fields: {
                                                ...
                                                TimeSheet: [{
                                                    ...
                                                    fields: {
                                                        ...
                                                        Allowedit: true,
                                                        // ChargeRate: 100,
                                                        Hours: parseFloat(templateObject.getActualHours()),
                                                        // OverheadRate: 90,
                                                        Job: clientname || "",
            ...
            if (logid != null && logid > 0) {
                ...
                appointmentService.saveTimeLog(obj).then(function(result) {
                    if (obj.fields.Description == "Job Completed") {
                        let data = {
                            type: "TTimeSheetEntry",
                            fields: {
    ...
    'click .btn-auto-save-calendar': async function(event) {
        let currentId = FlowRouter.current().context.hash
        let templateObject = Template.instance();
...
openAppointModalDirectly = (leadid, templateObject, auto = false) => {
    ...
    if (FlowRouter.current().queryParams.leadid) {
        contactService.getOneLeadDataEx(leadid).then(function(data) {
            ...
            let getEmployeeID = templateObject.empID.get() || "";
            document.getElementById("employee_name").value = localStorage.getItem("mySessionEmployee");
            document.getElementById("customer_appointment_calendar").value = data.fields.ClientName;
