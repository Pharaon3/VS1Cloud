...
Template.journalentrycard.onRendered(() => {
                        ...
                        for (let d = 0; d < useData.length; d++) {
                            if (parseInt(useData[d].fields.ID) === currentBill) {
                                ...
                                setTimeout(function() {
                                    ...
                                    $("#dtTransDate").datepicker('setDate',  new Date(useData[d].fields.TransactionDate));
                                }, 200);
    ...
    exportSalesToPdf1 = async function(template_title,number) {
        if(template_title == 'Journal Entry')
        {
...
Template.journalentrycard.events({
    ...
    'click .printConfirm': async function(event) {
        ...
        setTimeout(async function(){
            ...
            if($('#print_journalentry').is(':checked')) {
                printTemplate.push('Journal Entry');
            }

            if(printTemplate.length > 0) {
                for(var i = 0; i < printTemplate.length; i++)
                {
                    if(printTemplate[i] == 'Journal Entry')
                    {
                        var template_number = $('input[name="JournalEntry"]:checked').val();
                    }
