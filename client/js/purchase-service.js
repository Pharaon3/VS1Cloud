...
export class PurchaseBoardService extends BaseService {
    ...
    getPurchaseOrderByWorkorderid(id) {
        let options = {
            ListType: 'Detail',
            select: "[deleted]=false and [CustField3]='" +id +"'"
        }
    ...
    savePurchaseOrderBackOrder(data) {
        return this.POST(this.ERPObjects.TpurchaseOrderBackOrder, data);
    }
  ...
  getChequeList(dateFrom, dateTo, ignoreDate, limitCount, limitFrom, deleteFilter) {
      let options = "";
      if (ignoreDate == true) {
          options = {
              ListType: "Detail",
    ...
    getChequeListByAccountName(accountName) {
        let options = "";
        options = {
            ListType: "Detail",
            IgnoreDates: true,
            Search: "Deleted = 'F' and Account='" + accountName + "'",
        };
        return this.getList(this.ERPObjects.TChequeList, options);
    }
