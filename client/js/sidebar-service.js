...
export class SideBarService extends BaseService {
  ...
  getTaxRateVS1ByName(dataSearchName) {
    let options = {
      ...
      Search: "Active = true and CodeName like '%" + dataSearchName + "%'"
    };
    return this.getList(this.ERPObjects.TTaxcodeVS1, options);
  }
  ...
  getManualTitleList() {
    ...
    getWeightUnits() {
          return this.getManualWeightUnits();

        }
        getManualWeightUnits() {
            return this.getWowWeightUnits();
        }
        getWowWeightUnits() {
            var that = this;
            var promise = new Promise(function(resolve, reject) {
                var splashArrayTitleList = [['1', 'T'],
                ['2', 'KG'],
                ['3', 'G'],
                ['4', 'MG']]
                resolve({"tweightunit" : splashArrayTitleList});
            });
            return promise;
        }

        getVolumeUnits() {
          return this.getManualVolumeUnits();

        }
        getManualVolumeUnits() {
            return this.getWowVolumeUnits();
        }
        getWowVolumeUnits() {
            var that = this;
            var promise = new Promise(function(resolve, reject) {
                var splashArrayTitleList = [['1', 'M3'],
                ['2', 'L'],
                ['3', 'CM3']]
                resolve({"tvolumeunit" : splashArrayTitleList});
            });
            return promise;
        }
