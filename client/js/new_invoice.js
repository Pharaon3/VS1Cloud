 ...
 */
    ...
    if (clientList) {
      for (var i = 0; i < clientList.length; i++) {
        if (clientList[i].customername == data.fields.CustomerName) {
          ...
          $("#edtCustomerAccountNo").val(clientList[i].accountNo || '');
          break;
        }
      }
    }
  ...
  templateObject.generateInvoiceData = async function (template_title, number) {
    ...
    if (template_title == 'Delivery Docket') {
      $(".horizontal_blue_line, .horizontal_black_line").hide();
      ...
      if (saledateTime) {
        ...
        $('.amountdue, .amountdue3').text(convertDateFormatForPrint2(saleDate));
      } else {
        $('.amountdue, .amountdue3').text('-');
      }
    } else {
      $(".horizontal_blue_line, .horizontal_black_line").show();
      $('.amountdue_label, .amountdue3_label').text('AMOUNT DUE');
    }
  };
...
Template.new_invoice.onRendered(function () {
  ...
  $(document).on("click", ".btnPreviewTemplate", function (e) {
