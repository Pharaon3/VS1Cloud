                ...
                $('#tblSearchOverview tbody').on('click', 'tr', function () {
                    ...
                    if ((listData) && (transactiontype)) {
                        if (transactiontype === 'Purchase Order') {
                            ...
                            window.open('/invoicetemp?id=' + listData, '_self');
                        ...
                        for (let i = 0; i < data.tglobalsearchreport.length; i++) {
                            if (data.tglobalsearchreport[i].Type === "Invoice") {
                                ...
                                if (dataSelectID != '' && dataSelectID == segs[1]) {
                                    window.open('/invoicetemp?id=' + dataSelectID, '_self');
                                }
                            } else {
                                swal('No record with that exact number "' + barcode + '"', '', 'warning');
                                $('.fullScreenSpin').css('display', 'none');
...
Template.header.events({
    ...
    /**
     * For Background Audio / Damien
     */
    'click .background-sound-on': function(e) {
        // startBackgroundAudio();
    },
    'click .background-sound-off': function(e) {
        // stopBackgroundAudio();
    },
    'click a.background-sound': function(e) {
        $("#volume-slider").fadeToggle();
