...
Template.creditcard.onRendered(() => {
  ...
  function showCreditData(template_title, number) {
    ...
    let customerAccountNo = $("#edtCustomerAccountNo").val();
    ...
    var duedate = invoice_data.duedate || dtSODate;
    let termAmount = $('#selectTermAmount').val();
    if (termAmount && dtSODate) {
      var parts = dtSODate.split('/');
      var date = new Date(parts[2], parts[1] - 1, parts[0]);
      date.setDate(date.getDate() + Number(termAmount));
            ...
            if (dataObject.length == 0) {
              ...
              for (let d = 0; d < useData.length; d++) {
                if (parseInt(useData[d].fields.ID) === currentCredit) {
                  ...
                  $("#dtSODate").datepicker('setDate',  new Date(useData[d].fields.OrderDate));
      ...
      if (taxItems && Object.keys(taxItems).length > 0) {
        ...
        for(item of data){
            ...
            for(item_temp of item){
                if (idx > 1)
                    html = html + "<td class=\"" + fieldKeys[idx] + "\" style='text-align: right; padding-right: " + firstIndentLeft + "px;'>" + item_temp + "</td>";
                else
                    html = html + "<td class=\"" + fieldKeys[idx] + "\" style='padding-left: " + firstIndentLeft + "px;'>" + item_temp + "</td>";
                idx++;
            }
            html +="</tr>";
