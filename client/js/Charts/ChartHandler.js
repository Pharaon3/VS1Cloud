...
export default class ChartHandler {
  ...
  static buildPositions() {
    const charts = $(".sortable-chart-widget-js");
  ...
  static async saveCharts(charts = $(".sortable-chart-widget-js")) {
       /**
    ...
    Array.prototype.forEach.call(charts, (chart) => {
      ...
      let chartID = parseInt($(chart).attr("chart-id"))
      if (!chartID) chartID = 0
      chartList.push({
        type: "Tvs1charts",
        fields: {
          Active: $(chart).find(".chartShowOption").prop("checked") ? true : false,
          // ChartID: parseInt($(chart).attr("chart-id")),
          ID: chartID, // This is empty when it is the first time, but the next times it is filled
          // EmployeeID: localStorage.getItem("mySessionEmployeeLoggedID"),
          ChartName: $(chart).attr("key"),
          // Position: parseInt($(chart).attr("position")),
          ChartGroup: $("#connectedSortable").data("chartgroup"),
