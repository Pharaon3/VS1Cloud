...
Template.inventorylist.onCreated(function () {
  ...
  let headerStructure = [
    { index: 0, label: "ID", class: "colProductID colProuctPOPID", width: "10", active: false, display: true },
    ...
    { index: 8, label: "Cost Price (Ex)", class: "colCostPrice", width: "130", active: false, display: true },
    { index: 9, label: "Cost Price (Inc)", class: "colCostPriceInc", width: "135", active: true, display: true },
    { index: 10, label: "Sale Price (Ex)", class: "colSalePrice", width: "130", active: false, display: true },
...
Template.inventorylist.events({
  ...
  "click td.colAvailable": function (event) {
    var listData = $(event.target).closest("tr").attr('id');
    var listProductName = $(event.target).closest("tr").find(".colInventoryProductName").text();
    if (listData) {
      ...
      setTimeout(() => {
        $('#productrecentlist1_filter .form-control-sm').get(0).focus();
  ...
  "click td.colOnSO": function (event) {
    ...
    if (listData) {
      $("#onSalesOrderPopUp #transTitle2").text(listProductName + " - On Sales Order");
      let templateObject = Template.instance();
      templateObject.productID.set(listData);
  ...
  "click td.colOnOrder": function (event) {
    ...
    var listProductName = $(event.target).closest("tr").find(".colInventoryProductName").text();
    if (listData) {
      $("#onOrderPopUp #transTitle1").text(listProductName + " - On Order");
  ...
  "click th.colSalePrice": function (event) {
    ...
    // $(".chkSalePrice").prop("checked", false);
    // $(".chkSalePriceInc").prop("checked", true);
    $('.chkcolSalePriceInc').click();
    $('.chkcolSalePrice').click();
  },
  "click th.colSalePriceInc": function (event) {
    // $(".colSalePriceInc").addClass("hiddenColumn");
    // $(".colSalePriceInc").removeClass("showColumn");
  ...
  //   }
  // },
  'click #setUpInventoryList':function(){
    localStorage.setItem('VS1Cloud_SETUP_STEP',10)
