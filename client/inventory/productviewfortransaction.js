import "./productviewfortransaction.html";
...
Template.productviewfortransaction.onCreated(() => {
...
Template.productviewfortransaction.onRendered(function () {
  $(".fullScreenSpin").css("display", "inline-block");
  ...
  var currentProductID = templateObject.data.id;
  if(templateObject.data.isModal == true) {
    currentProductID= templateObject.data.id
  }
  ...
  if (templateObject.data.id || templateObject.data.name) {
    ...
    async function getProductDataByID (id) {
      return new Promise((resolve, reject)=> {
        getVS1Data('TProductVS1').then(function (dataObject) {
          if(dataObject.length == 0) {
            productService.getOneProductdata(id).then(function (data) {
              resolve(data)
            }).catch(function(e) {
              resolve()
            })
    ...
    async function getProductDataByName(name) {
      return new Promise((resolve, reject)=> {
        getVS1Data('TProductVS1').then(function (dataObject) {
          if(dataObject.length == 0) {
            productService.getOneProductdatavs1byname(name).then(function (data) {
              resolve(data.tproductvs1[0])
            }).catch(function(e) {
              resolve()
            })
          } else {
    ...
    templateObject.getProductData = async function () {
      let productData;
      if(currentProductID) {
        productData = await getProductDataByID(currentProductID)
      } else if(templateObject.data.name) {
        productData = await getProductDataByName(templateObject.data.name);
      ...
      if(productData) {
        ...
        $(".fullScreenSpin").css("display", "none");
              ...
              let bomIndex = bomProducts.findIndex((product) => {
                return data.ProductName == product.Caption;
              });
      ...
      //       for (let i = 0; i < useData.length; i++) {
      //         if (parseInt(useData[i].fields.ID) === currentProductID || useData[i].fields.ProductName == templateObject.data.name) {
      //           // add to custom field
      //           // tempcode
      //           // setTimeout(function () {
...
Template.productviewfortransaction.helpers({
...
Template.productviewfortransaction.events({
  ...
  "click #btnSave": async function () {
   ...
   // let getIsManufactured = (await templateObject.isManufactured.get()) || false;
    ...
    setTimeout(async function () {
      ...
      var currentID = templateObject.data.id || 0;
  ...
  "click .btnDeleteInv": function (event) {
    ...
    swal({
      ...
      if (result.value) {
        ...
        var url = FlowRouter.current().path;
        var getso_id = url.split("?id=");
        var currentProduct = templateObject.data.id || "";
        var objDetails = "";
        if (getso_id[1] || templateObject.data.id) {
  ...
  "change #chkBOM": function (event) {
    ...
    if ($("#chkBOM").is(":checked")) {
      ...
      setTimeout(() => {
        if (templateObject.data.id || $("#edtProcess").val() == "") {
...
Template.productviewfortransaction.creatingNewTopic = function(){
  if(Session.get('is_creating_new_topic')){
    return true;
  }else{
    return false;
