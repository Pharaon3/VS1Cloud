...
Template.uomlistpop.events({
  "click #tblUOMListPop tbody tr": (e) => {        
    const uomName = $(e.currentTarget).find("td.colUOMName").text();
    $(e.currentTarget).parents(".modal").modal("hide");
    $("#sltUomPurchases").val(uomName);
    $("#sltUomPurchases").attr("value", uomName);
    $("#sltUomPurchases").trigger("change");
    $("#sltUomSales").val(uomName);
    $("#sltUomSales").attr("value", uomName);
    $("#sltUomSales").trigger("change");
  },
});
...
Template.uomlistpop.helpers({
  ...
  tablename: () => {
    ...
    return 'tblUOMListPop'+selCustID;
  },
});
