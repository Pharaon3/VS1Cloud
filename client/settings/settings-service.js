    ...
    getFormData(empData) {
        return new Promise((resolve, reject) => {
            ...
            $.grep(formsData, async (n) => {
                ...
                doneCount += 1;
                // if (count === doneCount) resolve(employeeScheduledRecord);
            });
            resolve(employeeScheduledRecord);
        });
    }

    getAutomatedEmail(limitCount, limitFrom, deleteFilter) {
        let that = this
        return new Promise(resolve => {
            that.getScheduleSettings()
                .then(async function (data) {
                    let empData = data.treportschedules;
                    let employeeScheduledRecord = await that.getFormData(empData)
                    if (!deleteFilter) {
                        resolve({treportschedules: employeeScheduledRecord})
                    } else {
                        resolve({treportschedules: employeeScheduledRecord, Params: {Search: ""}})
                    }
                })
        })
    }

    searchAutomatedEmail(dataSearchName) {
        let that = this
        return new Promise(resolve => {
            that.getScheduleSettings()
                .then(async function (data) {
                    let empData = data.treportschedules;
                    let employeeScheduledRecord = await that.getFormData(empData)
                    const filteredData = employeeScheduledRecord.filter(item => new RegExp(dataSearchName, "i").test(item.formname) || new RegExp(dataSearchName, "i").test(item.recipients))
                    resolve({treportschedules: filteredData})
                })
        })
    }
}
