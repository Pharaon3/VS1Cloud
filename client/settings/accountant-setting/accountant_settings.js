...
function initEditSupplierForm() {
    ...
    if (
        data.fields.Street == data.fields.BillStreet &&
        data.fields.Street2 == data.fields.BillStreet2 &&
        data.fields.State == data.fields.BillState &&
        data.fields.Postcode == data.fields.Postcode &&
...
Template.accountantsettings.onCreated(() => {
...
Template.accountantsettings.onRendered(function () {
    $(".fullScreenSpin").css("display", "inline-block");
    const templateObject = Template.instance();
    let countries = [];
    ...
    templateObject.getOrganisationDetails = async () => {
        ...
        if (mainData.Address == mainData.PoBox && mainData.City == mainData.PoCity && mainData.State == mainData.PoState && mainData.Postcode == mainData.PoPostcode && mainData.Country == mainData.PoCountry) {
            templateObject.isSameAddress.set(true);
    ...
    $(document).ready(function () {
        ...
        $("#sltAccountant")
            .editableSelect()
            .on("click.editable-select", function (e) {
                var $earch = $(this);
    ...
    $(document).on("click", "#tblSupplierlist tbody tr", async function () {
...
Template.accountantsettings.helpers({
    showMob: () => {
...
Template.accountantsettings.events({
    "click #mobField": function () {
    ...
    "click #saveCompanyInfo": function () {
        playSaveAudio();
    ...
    "keyup #physicaladdress": function () {
        let templateObject = Template.instance();
    ...
    "click #uploadImg": async function () {
        //let imageData= (localStorage.getItem("Image"));
        let templateObject = Template.instance();
        ...
        if (imageData != null && imageData != "") {
            ...
            organisationService.saveCompanyLogo(companyLogoObj).then(function () {});
        }
    },
    ...
    "change #edtCountry": function (event) {
        if (event.target.value == "United States") {
            ...
            swal("Ooops...", "Can't Alter Country Once the Database Is Set.", "error");
